...
const featureKeys: Record<FeatureType, { title: TranslationKey; desc: TranslationKey }> = {
  'pdf-merge': { title: 'pdfMerge', desc: 'pdfMergeDesc' },
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {features.map((feature) => (
            ...
            >
              ...
              >
                {t(featureKeys[feature.id].title)}
              </h3>
              <p className="mt-0.5 text-xs text-gray-500">{t(featureKeys[feature.id].desc)}</p>
            </button>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900 mb-6">
            {t(featureKeys[activeFeature].title)}
          </h2>
          {renderFeature()}
        </div>
